...
- 🧠 **Intelligent Grouping**: Groups related messages across threads and channels using AI and semantic similarity
- 🚫 **Smart Filtering**: Filters out casual chat and irrelevant messages
- 📊 **Clean Dashboard**: Modern Neumorphism UI with real-time updates
- ✅ **Zero Duplicates**: Built-in de-duplication ensures no duplicate tickets
- 🏷️ **Message-Level Categories**: Each message tagged with its category (bug/feature/support/question)
- 📝 **AI-Generated Titles**: Concise, descriptive ticket titles generated by GPT-4o-mini
...
**System Components:**
- **Backend (Python)**: Receives Slack events via Socket Mode using slack-bolt, processes messages through AI classification and grouping, stores in Supabase
...
**Examples:**
- **Thread-based**: Message in thread → Always groups with parent ticket
- **AI-based**: "Need CSV export" (Monday) + "I don't see CSV button" (Wednesday) → Groups together
- **Semantic**: "Login broken" + "Login not working" (within 60 min) → Groups if similarity > 0.75
...
│   │   ├── grouping_classifier.py # AI-based grouping
...
│   │   └── TicketHistory.tsx   # History modal
│   ├── hooks/
...
└── database/
    ...
    ├── schema_updates.sql      # Status/history updates
    ├── add_message_category.sql # Message categories
